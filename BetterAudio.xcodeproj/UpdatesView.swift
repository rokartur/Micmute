...
struct UpdatesView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                CustomSectionView(title: "Update status", subtitle: "Manually check for new BetterAudio releases") {
                    updateStatusSection
                }

                CustomSectionView(title: "Automatic checks", subtitle: "Choose how often BetterAudio looks for updates") {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    HStack(alignment: .center, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("BetterAudio needs to restart to finish installing the update.")
                                .font(.system(size: 12))
                                .foregroundStyle(.secondary)
                        }
                    }
    ...
    private var automaticChecksSection: some View {
        ...
        return VStack(alignment: .leading, spacing: 16) {
            ...
            if updatesModel.frequency != .never {
                ...
                Label {
                    Text("BetterAudio won't check automatically — you'll need to run manual checks.")
                        .font(.system(size: 11))
                } icon: {
    ...
    private var statusSubtitle: String {
        ...
        return "BetterAudio is running the latest available build."
    }
    ...
    private var automaticChecksSummary: String {
        ...
        case .daily:
            return "BetterAudio checks for updates every day in the background."
        case .weekly:
            return "BetterAudio checks for updates once each week."
        case .monthly:
            return "BetterAudio checks for updates once each month."
        }
    }
    ...
    private var emptyState: some View {
        VStack(spacing: 10) {
            ...
            Text("Release notes are still loading")
                .font(.system(size: 14, weight: .semibold))
            Text("They will appear here once BetterAudio fetches them from GitHub.")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
